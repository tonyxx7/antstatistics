...
public class AntStatisticsLogger extends DefaultLogger {
    ...
    private static final String TABLE_HEADER_TARGET = "Target";
    private static final String TABLE_HEADER_DURATION = "Duration";
    ...
    private static final Comparator<ProjectPerformance> PROJECT_PERFORMANCE_COMPARATOR
			= new Comparator<ProjectPerformance>() {
        /** {@inheritDoc} */
        @Override
    ...
    private static final Comparator<TargetPerformance> TARGET_PERFORMANCE_COMPARATOR
			= new Comparator<TargetPerformance>() {
        /** {@inheritDoc} */
        @Override
    ...
    public void buildFinished(BuildEvent buildEvent) {
        ...
        Collections.sort(projects.getProjects(), PROJECT_PERFORMANCE_COMPARATOR);

        if (historyExpire > 0 && projects.getProjects().size() > historyExpire) {

            List<ProjectPerformance> retain = new ArrayList<ProjectPerformance>(projects.getProjects()
                    .subList(0, projects.getProjects().size() - historyExpire));
    ...
    private LinkedHashMap<Date, Map<String, Long>> createChartData(ProjectPerformances projects) {
    ...
    private void createStatisticsTable() {
        ...
        Collections.sort(targets, TARGET_PERFORMANCE_COMPARATOR);
        ...
        for (TargetPerformance target : targets) {
            ...
            double percentage = 100.0 * target.getDuration() / totalTargetDuration;
    ...
    private void initializeProperties(BuildEvent buildEvent) {
        Hashtable<String, Object> t = buildEvent.getProject().getInheritedProperties();
        t.toString();
    ...
    public void targetFinished(BuildEvent buildEvent) {
        TargetPerformance target = getTarget(buildEvent);

        if (target == null) {
            log(String.format("target '%s' not found", buildEvent.getTarget().getName()));
        } else {
            target.stopTimer();
        }
    }
    ...
    private void writeProjectPerformances(ProjectPerformances projects) {
        ...
        try {
            ...
            getXStream().toXML(projects, writer);
        } catch (Exception e) {
